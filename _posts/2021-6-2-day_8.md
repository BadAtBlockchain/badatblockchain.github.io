...
Sadly, based on testing these values all appeared to cap quite quickly and would ultimately result in clashes (this is due to the limit of 255 possibilities). If there were only a couple of hundred choices that needed to be randomised, then it would be perfect for selecting anything between 0 and 255. From here, the value can be [modded](https://en.wikipedia.org/wiki/Modulo_operation) to cap the minimum and maximum value brackets even more.
